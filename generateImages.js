...
module.exports = (visualizers, frames, config) => {
    ...
    while (frames.length) {
        ...
        var progressChars = Math.floor((i / numFrames) * progressWidth)
        if(progressChars - 3 < 0) {
            console.log(`\x1b[1A\x1b[2K\r\x1b[36m${"█".repeat(progressChars)}\x1b[0m${Math.floor((i / numFrames) * 100)}%\x1b[0m\x1b[36m${"░".repeat((progressWidth - progressChars) - (Math.floor((i / numFrames) * 100).toString().length + 1))}\x1b[0m`)
        } else {
            console.log(`\x1b[1A\x1b[2K\r\x1b[36m${"█".repeat(progressChars - 3)}\x1b[0m\x1b[46m\x1b[38;2;255;255;255m${Math.floor((i / numFrames) * 100).toString().padStart(2, " ")}%\x1b[0m\x1b[36m${"░".repeat(progressWidth - progressChars)}\x1b[0m`)
        }
        // console.log(`\x1b[1A\x1b[2K\rDone with ${i}/${numFrames} frames...`)
        i++
    }
